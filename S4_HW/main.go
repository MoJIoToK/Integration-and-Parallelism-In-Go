...
func main() {
	...
	n.network[nick.Id] = &nick

	n.network[vasya.Id] = &vasya
	...
	r.Get("/user/", n.GetAll)
	r.Post("/make_friends/{source}-{target}", n.MakeFriends)
	r.Delete("/delete/{id}", n.Delete)
	r.Get("/friends/{id}", n.GetFriends)
	r.Put("/update_age/{id}-{age}", n.UpdateAge)
